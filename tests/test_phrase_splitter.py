...
def test_split_text_detailed_true(splitter):
    specific_cases = [["寝ている", [["寝", "て"], ["いる"]]]]
    for input_string, expected in specific_cases:
        detailed_phrases = splitter.split_text(
            input_string,
            PhraseSplitter.OUTPUT_DETAILED,
            consider_non_independent_nouns_and_verbs_as_breaks=True,
        )
        surface_forms_only = [
            [token["surface_form"] for token in phrase] for phrase in detailed_phrases
        ]
