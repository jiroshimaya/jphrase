...
class PhraseSplitter:
    ...
    def create_tokenizer_from_mecab_ipadic(self, tagger: MeCab.Tagger) -> callable:
        ...
        def tokenize(text: str) -> list[dict]:
            ...
            mecab_result = mecab_result[
                :-1
            ]  # Remove the last line as it is unnecessary
            tokens = []
    ...
    def __should_break_before_token(
        ...
        pos_info = token["pos"]
        pos_detail = ",".join(
            [token["pos_detail_1"], token["pos_detail_2"], token["pos_detail_3"]]
        )

        previous_token = current_phrase[-1]
        previous_pos_info = previous_token["pos"]
        previous_pos_detail = ",".join(
            [
                previous_token["pos_detail_1"],
                previous_token["pos_detail_2"],
                previous_token["pos_detail_3"],
            ]
        )
        ...
        is_phrase_break_pos = (
            pos_info in phrase_break_pos_tags or token["surface_form"] in open_bra
        )
        if (
            not consider_non_independent_nouns_and_verbs_as_breaks
            and "非自立" in pos_detail
        ):
            is_phrase_break_pos = False

        if is_phrase_break_pos:
            if (
                previous_pos_info == "接頭詞"
                or previous_token["surface_form"] in open_bra
            ):
                return False
            elif "接尾" in pos_detail:
                return False
            elif (
                token["conjugated_type"] == "サ変・スル"
            ...
            if (
                previous_pos_info == "記号"
                and previous_token["surface_form"] not in close_bra
            ):
                return True
    ...
    def __split_text_into_detailed_phrases(
        ...
        for token in tokens:
            should_break = self.__should_break_before_token(
                token,
                current_phrase,
                consider_non_independent_nouns_and_verbs_as_breaks,
            )
    ...
    def __split_text_into_surface_phrases(
        ...
        surface_phrases = [
            "".join(token["surface_form"] for token in phrase)
        ...
        for phrase in detailed_phrases:
            concatenated_phrase = {
                ...
                "pronunciation": "".join(
                    token["pronunciation"]
                    for token in phrase
                    if token["pronunciation"] and token["pos"] != "記号"
                ),
                "reading": "".join(
                    token["reading"]
                    for token in phrase
                    if token["reading"] and token["pos"] != "記号"
                ),
            }
    ...
    def split_text(
        ...
        if output_type in split_methods:
            return split_methods[output_type](
                text, consider_non_independent_nouns_and_verbs_as_breaks
            )
        else:
